                                    ...
                                    {/* {output.value ? (
                                        <div className="rounded bg-white text-black text-sm max-h-[200px] overflow-auto border border-gray-200">
                                            <div className="sticky top-0 z-10 bg-white font-bold border-b border-gray-300">
                                                <div className="flex">
                                                    {output.value.columns?.map((col: string) => (
                                                        <div key={col} className="flex-1 px-2 py-1 border-r border-gray-200">
                                                            {col}
                                                        </div>
                                                    )) ?? []}
                                                </div>
                                            ...
                                            <List
                                                height={160}
                                                itemCount={output.value.shape[0]}
                                                itemSize={32}
                                                width="100%"
                                            >
                                                {({ index, style }) => {
                                                    //const rowDf = output.value?.iloc({ rows: [index] }); // Safe alternative to .row()
                                                    //const row = output.value.columns.map(col => rowDf.column(col).values[0]); // Extract row values
                                                    console.log("Rendering row:", index, output.value);
                                                    if (!output.value || output.value.shape[0] === 0) {
                                                        return (
                                                    ...
                                                    const row = output.value.$data[index];
                                                    return (
                                                        <div style={style} className="flex border-b border-gray-100 hover:bg-gray-100">
                                                            {row.map((cell: any, i: number) => (
                                                                <div key={i} className="flex-1 px-2 py-1 border-r border-gray-200 truncate">
                                                                    {String(cell)}
                                                                </div>
                                                            ))}
                                                        </div>
